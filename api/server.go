...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	if existingTrader.UserID != userID {
		c.JSON(http.StatusForbidden, gin.H{"error": "无权限操作其他用户的交易员"})
		return
	}
...
func (s *Server) handleCreateAIModel(c *gin.Context) {
	...
	// 解密敏感字段（如果前端加密了）
	req.APIKey = decryptIfEncrypted(req.APIKey)
...
func (s *Server) handleCreateExchange(c *gin.Context) {
	...
	// 解密敏感字段（如果前端加密了）
	req.APIKey = decryptIfEncrypted(req.APIKey)
	req.SecretKey = decryptIfEncrypted(req.SecretKey)
	req.HyperliquidWalletAddr = decryptIfEncrypted(req.HyperliquidWalletAddr)
	req.AsterPrivateKey = decryptIfEncrypted(req.AsterPrivateKey)
...
func (s *Server) handleGetTraderConfig(c *gin.Context) {
	...
	traderConfig, err := s.database.GetTraderConfig(userID, traderID)
	if err != nil {
...
func (s *Server) handleGetSupportedModels(c *gin.Context) {
	...
	models, err := s.database.GetSupportedAIModels()
	if err != nil {
...
func (s *Server) handleGetSupportedExchanges(c *gin.Context) {
	...
	exchanges, err := s.database.GetSupportedExchanges()
	if err != nil {
...
func (s *Server) Start() error {
	...
	log.Printf("  • POST /api/models           - 创建AI模型配置")
	log.Printf("  • PUT  /api/models/:model_id - 更新AI模型配置")
	log.Printf("  • DELETE /api/models/:model_id - 删除AI模型配置")
	log.Printf("  • GET  /api/exchanges        - 获取交易所配置")
	log.Printf("  • POST /api/exchanges        - 创建交易所配置")
	log.Printf("  • PUT  /api/exchanges/:exchange_id - 更新交易所配置")
	log.Printf("  • DELETE /api/exchanges/:exchange_id - 删除交易所配置")
