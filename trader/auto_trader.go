...
// SetScanInterval 设置扫描间隔（需要重启交易员才能生效）
func (at *AutoTrader) SetScanInterval(minutes int) {
	at.config.ScanInterval = time.Duration(minutes) * time.Minute
	log.Printf("⚙️  更新扫描间隔为 %d 分钟（需要重启交易员才能生效）", minutes)
}
